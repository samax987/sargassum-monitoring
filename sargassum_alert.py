...
Logique anti-spam : un message n'est envoye que si le risque a change
depuis la derniere alerte envoyee (stocke dans alert_state en DB).

Usage
-----
  python sargassum_alert.py            # verifie et envoie si necessaire
  python sargassum_alert.py --test     # envoie un message de test
  python sargassum_alert.py --force    # envoie meme si rien n'a change
"""
...
def _load_env(path: Path) -> dict:
    """Charge les variables d'un fichier .env sans ecraser l'environnement."""
    env = {}
    if not path.exists():
        return env
...
if not TELEGRAM_TOKEN or not TELEGRAM_CHAT:
    print("  [ERROR] TELEGRAM_TOKEN / TELEGRAM_CHAT manquants dans .env ou l'environnement.")
    sys.exit(1)

# Seuil minimum pour declencher une alerte (none < low < medium < high)
ALERT_MIN_RISK = "medium"

# Jours a surveiller (j+0 = aujourd'hui, j+1 = demain)
ALERT_DAY_OFFSETS = [0, 1]

# Ile prioritaire : beaches detaillees ligne par ligne (sinon resume compact)
PRIORITY_ISLAND = "Saint-Barth"

# Fenetre d'envoi matin (heure UTC) -- l'alerte ne part que lors du run de 06h
# pour eviter 4 messages par jour. --force et --test ignorent cette contrainte.
ALERT_HOUR_UTC = 6   # run de 06:00 UTC
ALERT_TOLERANCE = 1  # +/- heures acceptees (couvre les legers decalages cron)
...
def last_alert_hash(conn: sqlite3.Connection):
    row = conn.execute(
        "SELECT payload_hash FROM alert_state ORDER BY id DESC LIMIT 1"
    ).fetchone()
...
def load_all_beaches(conn: sqlite3.Connection) -> list:
    """Retourne tous les scores j+0 et j+1, toutes iles. Garde le pire jour par plage."""
...
def computed_at_latest(conn: sqlite3.Connection):
    row = conn.execute(
...
def build_message(all_beaches: list, computed_at: str) -> str:
    """Format compact par ile. Ile prioritaire detaillee, autres en resume."""
    ...
    if PRIORITY_ISLAND in by_island:
        ...
        if alert_bs:
            lines.append(f"📍 *{PRIORITY_ISLAND}* — a verifier :")
            for b in alert_bs:
                ...
                lines.append(f"  {icon} {name} ({day}, a {prox})")
        else:
    ...
    for island in sorted(by_island.keys()):
        ...
        for lvl in ("high", "medium", "low", "none"):
            if counts[lvl]:
                parts.append(f"{RISK_ICONS[lvl]}x{counts[lvl]}")
...
def main():
    ...
    if test:
        ok = send_telegram(
            ...
            "Les alertes seront envoyees ici quand une plage passe en risque moyen ou fort."
        )
        print("  OK Message test envoye" if ok else "  [ERROR] Echec envoi test")
        return

    if not force:
        now_hour = datetime.now(timezone.utc).hour
        diff = abs(now_hour - ALERT_HOUR_UTC)
        diff = min(diff, 24 - diff)
        if diff > ALERT_TOLERANCE:
            print(f"  [INFO] Hors fenetre d'alerte ({now_hour}h UTC, fenetre : {ALERT_HOUR_UTC}h +/-{ALERT_TOLERANCE}h) — pas d'envoi.")
            return
    ...
    if not computed_at:
        print("  [WARN] Aucun score de plage en base — pas d'alerte.")
        conn.close()
        return
    ...
    if not force and payload_hash == last_hash:
        print("  [INFO] Situation inchangee depuis la derniere alerte — pas d'envoi.")
        conn.close()
        return
    ...
    if alert_beaches:
        ...
        label = "situation degagee"

    ok = send_telegram(msg)
    if ok:
        save_alert(conn, computed_at, payload_hash)
        print(f"  OK Alerte Telegram envoyee — {label}")
    else:
        print("  [ERROR] Echec envoi Telegram")
